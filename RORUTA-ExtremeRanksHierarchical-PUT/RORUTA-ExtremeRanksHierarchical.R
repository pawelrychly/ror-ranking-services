##########################################
# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < RORUTA-ExtremeRanksHierarchical.R
# Example: 
# R --slave --vanilla --args "${PWD}/in" "${PWD}/out" < RORUTA-ExtremeRanksHierarchical.R
##########################################
...
if (is.null(errFile) && is_proper_data){
  tmpErr<- try (
    {
      results <- findExtremeRanksHierarchical(
        ...
        strict.vf=strict, nums.of.characteristic.points=nums.of.characteristic.points, criteria=criteria.preference.directions, hierarchy.data=hierarchy.data$data) 
    }, silent=TRUE
  )  
