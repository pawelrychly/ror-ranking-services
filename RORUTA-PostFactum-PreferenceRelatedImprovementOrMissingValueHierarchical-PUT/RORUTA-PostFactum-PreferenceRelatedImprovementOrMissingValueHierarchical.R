##########################################
# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < RORUTA-PostFactum-PreferenceRelatedImprovementOrMissingValueHierarchical.R
# Example: 
# R --slave --vanilla --args "${PWD}/in" "${PWD}/out" < RORUTA-PostFactum-PreferenceRelatedImprovementOrMissingValueHierarchical.R
##########################################
  ...
  tmpErr<- try (
    ...
    if (possible.or.necessary == "necessary") {
        if (type.of.result == "missing-value") {
          results <- findMissingOrRedundantUtilityValueForRelationHierarchical(
          ...
          results <- findPerformanceModificationForNecessaryRelationHierarchical(
            perf=performances$data, a = target.variant.a, b = target.variant.b,
            strict.vf = strict,
            strong.prefs = preferences$strong, weak.prefs=preferences$weak, 
            indif.prefs=preferences$indif, 
            ...
            greater.than.one = TRUE, hierarchy.data=hierarchy.data)
        } 
      } else {
        if (type.of.result == "missing-value") {
          results <- findMissingOrRedundantUtilityValueForRelationHierarchical(
          ...
          results <- findPerformanceModificationForPossibleRelationHierarchical(
            perf=performances$data, a = target.variant.a, b = target.variant.b,
            strict.vf = strict,
            strong.prefs = preferences$strong, weak.prefs=preferences$weak, 
            indif.prefs=preferences$indif, 
...
if (execFlag) {
  ...
  newXMLNode("xmcda:XMCDA",
             ...
             parent=tree)
  target.variant.a <- which(rownames(performances$data) == target.variant.a)
  for (nodeid in names(results)) {
    ...
    alternative.value <- matrix(data=c(target.variant.a, res), ncol=2)
