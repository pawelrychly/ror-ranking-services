##########################################
# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < RORUTA-RankAcceptabilityIndicesHierarchical.R
# Example: 
# R --slave --vanilla --args "${PWD}/in" "${PWD}/out" < RORUTA-RankAcceptabilityIndicesHierarchical.R
##########################################
...
#OUTPUT FILES:
result.file <- "rank-acceptability-indices-hierarchical.xml"
...
if (is.null(errFile) && is_proper_data){
  tmpErr<- try (
    {
      results <- findRankAcceptabilityIndicesHierarchical(
        ...
        strict.vf=strict, nums.of.characteristic.points=nums.of.characteristic.points, criteria=criteria.preference.directions, num.of.samples = number.of.samples, hierarchy.data=hierarchy.data$data) 
    }, silent=TRUE
  )  
...
if (execFlag) {
  ...
  for (nodeid in names(results)) {
    rorranking:::putMatrixAsAlternativesValues(tree=outTree, alternativesMatrix=results[[nodeid]], attributes=c("id"=nodeid))
  }
