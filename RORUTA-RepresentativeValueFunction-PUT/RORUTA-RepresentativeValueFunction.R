##########################################
# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < RORUTA-RepresentativeValueFunction.R
# Example: 
# R --slave --vanilla --args "${PWD}/in" "${PWD}/out" < RORUTA-RepresentativeValueFunction.R
##########################################
  ...
  tmpErr<- try (
    {
      results <- findRepresentativeValueFunction(
        ...
        criteria=criteria.preference.directions, is.compromise=compromise, nec.relations.matrix=nec.rel$data) 
    }, silent=FALSE
  )  
