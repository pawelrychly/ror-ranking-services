##########################################
# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < RORUTA-NecessaryAndPossiblePreferenceRelations.R
# Example: 
# R --slave --vanilla --args "${PWD}/in" "${PWD}/out" < RORUTA-NecessaryAndPossiblePreferenceRelations.R
##########################################
...
#OUTPUT FILES:
result.file.nec <- "necessary-relations.xml"
result.file.pos <- "possible-relations.xml"
...
if (is.null(errFile) && is_proper_data){
  tmpErr<- try (
    {
      results <- findNecessaryAndPossiblePreferenceRelations(
...
if (execFlag) {
  ...
  for (id1 in ids) {
    for (id2 in ids) {
      if (results$nec.relations[id1,id2]) {
        nec.rel <- c(id1, id2)
        nec.relations <- rbind(nec.relations, nec.rel)
      } 
      if (results$pos.relations[id1,id2]) {
        pos.rel <- c(id1, id2)
