##########################################
# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < RORUTA-PairwiseOutrankingIndices.R
# Example: 
# R --slave --vanilla --args "${PWD}/in" "${PWD}/out" < RORUTA-PairwiseOutrankingIndices.R
##########################################
...
#OUTPUT FILES:
result.file <- "pairwise-outranking-indices.xml"
...
if (is.null(errFile) && is_proper_data){
  tmpErr<- try (
    {
      results <- findRelationsAcceptabilityIndices(
...
if (execFlag) {
  ...
  for (id1 in ids) {
    for (id2 in ids) {
        pair.value <- c(id1, id2, results[id1,id2]);
        results.matrix <- rbind(results.matrix, pair.value);
    }
  }
  putAlternativesComparisonsLabels(outTree, results.matrix)
